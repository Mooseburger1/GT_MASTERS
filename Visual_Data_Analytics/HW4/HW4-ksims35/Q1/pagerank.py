...
"""
def author():                                                                                             
        return 'ksims35' # replace gburdell3 with your Georgia Tech username.                                                                                             
                                                                                              
def gtid():                                                                                               
    return 903469283 # replace with your GT ID number      
...
class PageRank:
    ...
    #method to check who is bigger 
    def which_is_bigger(self, vals):
        largest_idx = np.argmax(vals)
        return vals[largest_idx]
    '''
...
#############################################
           ...
           #track largest node id
           self.max_node_id = self.which_is_bigger([source, target, self.max_node_id])

           #increment node out degree on source
...
### Implement your code here
#############################################
                rhs = damping_factor * (scores[source] / self.node_degree[source])

                new_scores[target] += rhs

            update = [(1-damping_factor) * node_weights[pos] + x for pos, x in enumerate(new_scores)]
            scores = update.copy()    
