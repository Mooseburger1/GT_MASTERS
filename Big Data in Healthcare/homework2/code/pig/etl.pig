...
-- load events file
--events = LOAD 'hdfs://bootcamp.local:9000/user/scott/input/events/events.csv' USING PigStorage(',') AS (patientid:int, eventid:chararray, eventdesc:chararray, timestamp:chararray, value:float);
...
-- load mortality file
--mortality = LOAD 'hdfs://bootcamp.local:9000/user/scott/input/mortality/mortality.csv' USING PigStorage(',') as (patientid:int, timestamp:chararray, label:int);
...
--join events and mortality on patientid
eventswithmort = JOIN events BY patientid FULL OUTER, mortality BY patientid;

--filter out entries with a 1 in mortality label
deadevents = FILTER eventswithmort BY label == 1;
--create a relation table with patientid and the death timestamp
deaddates = FOREACH deadevents GENERATE $0, $5 AS dead_timestamp;
--Subtract 30 days from the death timestamp
deadindexdates = FOREACH deaddates Generate patientid, SubtractDuration(dead_timestamp, 'P30D') as indexdate;
--Get the distinct patientids
deadindexdates = DISTINCT deadindexdates;
--Join deadindexdates on deadevents by patient id
deadevents = JOIN deadevents BY $0, deadindexdates BY $0;
--select desired columns
deadevents = FOREACH deadevents GENERATE $0, $1, $2, $3, $6, $8;
--calculate days between event time stamp and index date
deadevents = FOREACH deadevents GENERATE $0, $1, $3, $4, DaysBetween($5, $2) AS time_difference;
...
-- filter out label entries that are null in eventswithmort
aliveevents = FILTER eventswithmort BY label IS NULL;
-- fill null values with 0 *bincond* {condition} ? {true response}:{false response}
aliveevents = FOREACH aliveevents GENERATE $0, $1, $2, $3, $4, $5, (label IS NOT NULL ? label:0) as label;
-- filter out event dates
alivedates = FOREACH aliveevents GENERATE $0, $2;
-- group by patientid, sort by timestamps descending and select first timestamp as index date
aliveindexdates = FOREACH (GROUP alivedates BY $0){
    sortdates = ORDER alivedates BY $1 DESC;
    lastdate = LIMIT sortdates 1;
    generate flatten(lastdate);
};
-- join index date back on aliveevents by patientid
aliveevents = JOIN aliveevents BY $0, aliveindexdates BY $0;
-- select desired columns
aliveevents = FOREACH aliveevents GENERATE $0, $1, $2, $3, $6, $8;
-- calculate days between
aliveevents = FOREACH aliveevents GENERATE $0, $1, $3, $4, DaysBetween($5, $2) AS time_difference;
...
-- ***************************************************************************
filteredalive = FILTER aliveevents BY (time_difference <= 2000) AND (time_difference >= 0);
filtereddead = FILTER deadevents BY (time_difference <= 2000) AND (time_difference >= 0);
filtered = FILTER (UNION filteredalive, filtereddead) BY value IS NOT NULL;
...
-- -- ***************************************************************************
filteredgrp = GROUP filtered BY ($0, $1);
featureswithid = FOREACH filteredgrp{
    cnt = COUNT(filtered);
    GENERATE flatten(group), flatten(cnt) AS featurevalue;
};
...
-- -- ***************************************************************************
events = FOREACH featureswithid GENERATE eventid as eventid;
events = RANK (ORDER (DISTINCT events) BY eventid);
all_features = FOREACH events GENERATE ($0 - 1) AS idx, eventid;
...
-- -- ***************************************************************************
maxvalues = FOREACH (GROUP features BY idx){
    m = MAX(features.featurevalue);
    GENERATE group AS idx, flatten(m) AS maxvalue;
};
...
features = FOREACH normalized GENERATE $0 AS patientid, $1 AS idx, ((DOUBLE) $2  / (DOUBLE) $4) AS normalizedfeaturevalue;
